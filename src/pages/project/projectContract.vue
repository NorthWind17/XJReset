...
export default {
    ...
    data() {
        return {
            ...
            fDrawerForm: {
                sgtitle: '',
                xmid: this.$store.state.projectInfo.uid,
                fjurl: [],
                image: [],
                spuser: [],
                csuser: [],
    ...
    methods: {
            ...
            this.$axios
                ...
                .then((res) => {
                    if (res.data.code == 200) {
                        ...
                        let newData = [];
                        let newChar = [];
                        res.data.stype.forEach((item) => {
                            let obj = {};
                            for (let i = 0; i <= res.data.stype.length; i++) {
                        ...
                        res.data.ztype.forEach((item) => {
                            let obj = {};
                            for (let i = 0; i <= res.data.ztype.length; i++) {
                                obj.value = item.count;
                                obj.name = item.name;
                                obj.money = item.money;
                            }
        ...
        getBasicList() {
            this.$axios
                ...
                .then((res) => {
                    if (res.data.code == 200) {
                        this.AbasicList = res.data.content.f;
                        this.BbasicList = res.data.content.g;
                    } else {
                    }
                })
                .catch(function (error) {
